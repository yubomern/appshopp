...
router.post("/create-user1", upload.single("file"), async (req, res, next) => {
  try {
    const { name, email, password } = req.body;
    const userEmail = await User.findOne({ email });
  ...
  catchAsyncErrors(async (req, res, next) => {
    try {
      ...
      /*if (phoneNumber.startsWith("0")) {
        phoneNumber = "+216" + phoneNumber.slice(1);
      }*/
     ...
     // phoneNumber = "+216" + phoneNumber.slice(1);
      ...
      const activationUrl = `http://localhost:1001/user/user-activation/${activationToken}`;
...
router.get(`/getuserid/:id`, isAuthenticated, async (req, res, next) => {
  try {
